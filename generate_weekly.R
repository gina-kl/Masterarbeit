...
simulate_scenario <- function(
  ...
  D <- D +
    ...
    # Baseline Fall-Risk (historic falls before entry), influenced by Frailty in scenarios 4 & 5
  ...
  Odat %<>%
    ...
    mutate(
      ...
      # Administrative censoring (affected by informative frailty only in scen 5)
      censor_rate = exp(cens_intercept + 0.02 * age + eff_frailty_cens * frailty), exp_followtime = rexp(1, rate = censor_rate[1]) + 1,
      entry_time = 0,
